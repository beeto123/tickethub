...
function App() {
  ...
  if (loading) return <div className="p-10 text-center text-2xl">Loading tickets...</div>;
  ...
  return (
    <div className="min-h-screen bg-gray-100">
      ...
      <div className="bg-gradient-to-r from-blue-600 to-purple-700 text-white py-10 sm:py-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-8 text-center">
          <h2 className="text-3xl sm:text-4xl md:text-5xl font-bold mb-2 sm:mb-4">Find Your Next Experience</h2>
          <p className="text-base sm:text-xl opacity-90">Tickets for sports, concerts & more</p>
        </div>
      </div>

      {/* Tickets Grid - Mobile Responsive */}
      <div className="max-w-7xl mx-auto px-4 sm:px-8 py-8 sm:py-12">
        <h2 className="text-2xl sm:text-3xl font-bold mb-6 sm:mb-10 text-gray-800">Available Tickets</h2>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 lg:gap-8">
          {tickets.map((ticket) => {
            ...
            const ticketUrl = `/ticket/${ticket.slug || ticket.id}`;
            
            return (
              <Link 
                to={ticketUrl} 
                key={ticket.id} 
                className="block bg-white rounded-xl sm:rounded-2xl shadow-lg hover:shadow-2xl transition duration-300 cursor-pointer overflow-hidden"
              >
                <div className="relative">
                  ...
                  />
                  {isSold && (
                    <div className="absolute top-3 right-3 sm:top-4 sm:right-4 bg-red-600 text-white px-3 py-1 sm:px-4 sm:py-2 rounded-full font-bold text-xs sm:text-sm">
                      SOLD OUT
                    </div>
                  )}
                </div>
                <div className="p-4 sm:p-6">
                  <h3 className="text-lg sm:text-xl font-bold mb-1 sm:mb-2 text-gray-800 line-clamp-2">{ticket.event_name}</h3>
                  <p className="text-sm sm:text-base text-gray-600 mb-1">📅 {ticket.date}</p>
                  <p className="text-sm sm:text-base text-gray-600 mb-3 sm:mb-4">📍 {ticket.location}</p>
                  ...
                  <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center border-t pt-3 sm:pt-4 gap-3 sm:gap-0">
                    ...
                    {!isSold && hasPaymentLink ? (
                      <div className="bg-[#004C9C] text-white px-4 sm:px-6 py-2 sm:py-3 rounded-xl font-semibold text-sm sm:text-lg hover:bg-blue-800 transition w-full sm:w-auto text-center">
                        View Details →
                      </div>
                    ) : isSold ? (
                      <div className="bg-gray-400 text-white px-4 sm:px-6 py-2 sm:py-3 rounded-xl font-semibold text-sm sm:text-lg w-full sm:w-auto text-center">
                        Sold Out
                      </div>
                    ) : (
                      <div className="bg-gray-400 text-white px-4 sm:px-6 py-2 sm:py-3 rounded-xl font-semibold text-sm sm:text-lg w-full sm:w-auto text-center">
